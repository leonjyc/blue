...
@property (nonatomic) NSString *cancleBtnTitleText;
...
+ (instancetype)alertViewWithTitle:(NSString *)title
    ...
    return [[GUAAlertView alloc] initWithTitle:title message:message buttonTitle:buttonTitle cancleTitle:cancleStr buttonTouchedAction:buttonAction dismissAction:dismissAction];
}
+ (instancetype)alertViewWithTitle:(NSString *)title
                           message:(NSString *)message
                       buttonTitle:(NSString *)buttonTitle
...
- (void)setupLayoutConstraints {
    ...
    if (self.cancleBtnTitleText.length > 0) {
...
//                                views:views]];
    if (self.cancleBtnTitleText.length > 0) {
        ...
        [_cancleBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(_messageLabel.mas_bottom).offset(20);
            make.left.equalTo(_alertView.mas_left);
            make.width.equalTo(_button.mas_width);
            make.height.equalTo(@44);
            make.right.mas_equalTo(_button.mas_left);
        }];
...
- (void)setupButton {
    ...
    if (_cancleBtnTitleText.length > 0) {
        ...
        _cancleBtn.layer.shadowColor = [[UIColor grayColor] CGColor];
        _cancleBtn.layer.shadowRadius = 0.5;
        _cancleBtn.layer.shadowOpacity = 1;
        _cancleBtn.layer.shadowOffset = CGSizeZero;
        _cancleBtn.layer.masksToBounds = NO;
        
        // background color
        [_cancleBtn setBackgroundImage:imageFromColor([UIColor grayColor])
                           forState:UIControlStateHighlighted];
        [_cancleBtn setBackgroundImage:imageFromColor([UIColor whiteColor])
                           forState:UIControlStateNormal];
        
        // title
        [_cancleBtn setTitle:_cancleBtnTitleText forState:UIControlStateNormal];
        [_cancleBtn setTitle:_cancleBtnTitleText forState:UIControlStateSelected];
        _cancleBtn.titleLabel.font = [UIFont boldSystemFontOfSize:_button.titleLabel.font.pointSize];
