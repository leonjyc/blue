 ...
 *  @return 转场动画实例
 */
-(CATransition *)transitionWithAnimType:(TransitionAnimType)animType subType:(TransitionSubType)subType curve:(TransitionCurve)curve duration:(CGFloat)duration resultStatus:(resultStatusBlock)block{
    ...
    block = block;
    [self performSelector:@selector(endAnim:) withObject:block afterDelay:duration];
    return transition;
}

- (void)endAnim:(resultStatusBlock)block{
    block(@"1");
}
